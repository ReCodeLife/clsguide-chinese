...
\title{\Huge {\heiti 用于类和包作者的}~\textbf{\LaTeXe}}
...
\end{decl}
这声明当前文件(current file)包含文档类 \m{class-name}\ 或宏包 \m{package-name}\ 的定义
...
\end{decl}
这意味着它将 \m{option-list}\ 添加到未来 |\RequirePackage| 或 |\usepackage| 命令使用的选项列表中，|\RequirePackage| 或 |\usepackage| 命令用于 \m{package-name}。
...
在 \m{error-text} 和 \m{help-text}\ 中：|\protect| 可用于阻止命令展开；|\MessageBreak| 会导致换行(line-break)；|\space| 会打印空格(space)。
